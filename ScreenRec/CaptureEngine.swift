        ...
        AsyncThrowingStream<CapturedFrame, Error> { continuation in
          ...
          //  streamOutput.pcmBufferHandler = { self.powerMeter.process(buffer: $0) }
            ...
            do {
                ...
                //print("Width: \(configuration.width) height: \(configuration.height) ")
                
                vr?.width = configuration.width
                vr?.height = configuration.height
...
private class CaptureEngineStreamOutput: NSObject, SCStreamOutput, SCStreamDelegate {
    ...
    func stream(_ stream: SCStream, didOutputSampleBuffer sampleBuffer: CMSampleBuffer, of outputType: SCStreamOutputType) {
        ...
        case .screen:
                ...
                //print("Prez buffer: \(sampleBuffer.presentationTimeStamp)")
...
/*
            ...
            */
            ...
            //vr?.append(sampleBuffer: sampleBuffer)
            let duration = self.lastSampleTime - sampleBuffer.presentationTimeStamp
            
            let timing = CMSampleTimingInfo(duration: duration, presentationTimeStamp: lastSampleTime, decodeTimeStamp: sampleBuffer.decodeTimeStamp)
                if let retimedSampleBuffer = try? CMSampleBuffer(copying: sampleBuffer, withNewTiming: [timing]) {
                    vr?.append(sampleBuffer: retimedSampleBuffer)
                } else {
                    print("Couldn't copy CMSampleBuffer, dropping frame")
                }
            self.lastSampleTime = sampleBuffer.presentationTimeStamp
